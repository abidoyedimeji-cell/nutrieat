-- NutriEat Platform — Wave 1D (PR3) hotfix: webhook suppression-reason enum cast + provider-event
-- linkage. The suppression 'reason' CASE needs an explicit enum cast. CREATE OR REPLACE (idempotent);
-- no schema change.
